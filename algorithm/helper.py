...
class Episode(object):
    ...
    def from_trajectory(cls, cfg, obs, states, action, reward, done=None):
        ...
        # for idx in range(cfg.episode_length):
        #     if episode.reward[idx] == 0.0:
        #         success_idx = idx
        #         break
        # episode.reward[success_idx+1:] = -1.0
        episode.cumulative_reward = torch.sum(episode.reward)
        # print('demo episode reward: ', episode.reward)
        # print('+++episode.cumulative_reward: ', episode.cumulative_reward)
        # print('episode.cumulative_reward: ', torch.sum(episode.reward))
        episode.done = True
...
# add by Yao-Hui Li
def calculate_MC_target(cfg, episode: Episode):
    # episode idx:  episode_length
    for i in range(cfg.episode_length - 1, -1, -1):
        if i ==cfg.episode_length - 1:
            x = episode.reward[i]
        else:
            x = episode.reward[i] + cfg.discount * x

        episode.MC_Q_target[i] = x




def get_demos(cfg):
    fps = glob.glob(str(Path(cfg.demo_dir) / f"{cfg.task}/0.pt"))
    episodes = []
    for fp in fps:
        data = torch.load(fp)
...
class ReplayBuffer(object):
    ...
    def sample(self):
        ...
        probs /= probs.sum()
        # print('probs是否含有nan: ', torch.isnan(probs).any())
        total = len(probs)
        ...
        # idxs = torch.from_numpy(
        #     np.random.choice(
        #         total, self.cfg.batch_size, replace=True
        #     )
        # ).to(self.device)
